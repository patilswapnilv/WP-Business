...
register_setting(
	// $option_group
	tm_get_current_theme_id() . '_options',
	// $option_name
	tm_get_current_theme_id() . '_options',
	// $sanitize_callback
	'tm_options_validate'
...
function tm_register_admin_js_globals(){
	...
	$selected_tab = $selected_tab ? $tm_tabs[0]['name'] : $tab;
	...
	if( isset( $_GET['page']) && esc_attr( $_GET['page'] ) == 'tm-settings' && $selected_tab )
...
add_action( 'admin_enqueue_scripts', 'tm_register_admin_js_globals', 1 );
...
function tm_options_validate( $input ) {
	...
	$valid_input = (array) tm_get_options();
	// Get the array of Theme settings, by Settings Page tab
	$settingsbytab = tm_get_settings_by_tab();
...
foreach ( $tm_tabs as $tab ) {
	...
	foreach ( $tabsections as $section ) {
		...
		add_settings_section( 'tm_' . $section['name'] . '_section', $section['title'], 'tm_sections_callback', 'tm_' . $tabname . '_tab' );
 ...
 */
function tm_sections_callback( $section_passed ) {
	global $tm_tabs;
	foreach ( $tm_tabs as $tabname => $tab ) {
		$tabsections = $tab['sections'];
		foreach ( $tabsections as $sectionname => $section ) {
			if ( 'tm_' . $sectionname . '_section' == $section_passed['id'] ) {
				?>
 ...
 */
function tm_setting_callback( $option ) {
	...
	$value = $tm_options[ $optionname ];

    //Determine the type of input field
    switch ( $fieldtype ) {

        //Render Text Input
        case 'text': tm_field_text( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render textarea options
        case 'textarea': tm_field_textarea( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render select dropdowns
        case 'select': tm_field_select( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render radio dropdowns
        case 'radio': tm_field_radio( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render radio image dropdowns
        case 'radio_image': tm_field_radio_image( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render checkboxes
        case 'checkbox': tm_field_checkbox( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render color picker
        case 'color': tm_field_color( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

        //Render uploaded image
        case 'image': tm_field_image( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;

         //Render uploaded gallery
        case 'gallery': tm_field_gallery( $value, $attr );
		echo '<span class="option-description">' . $option['description'] . '</span>';
        break;
