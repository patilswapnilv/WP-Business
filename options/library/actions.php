...
add_action( 'wp_head', 'tm_custom_css', 11);

/**
 * Google Font Integration
 */
function tm_include_font() {
...
add_action( 'wp_head', 'tm_include_font' );

/**
 * Alternative styles
 */
function  tm_alt_styles() {

    $theme_options = get_option( tm_get_current_theme_id() . '_options' );
	if ( isset ( $theme_options['color'] ) && '' != $theme_options['color'] ) {
		$file = get_stylesheet_directory() . '/css/' . $theme_options['color'] . '.css';
		if ( file_exists( $file ) ) {
			wp_enqueue_style( 'tm-alt-style', get_stylesheet_directory_uri() . '/css/' . $theme_options['color'] . '.css', array( 'style' ) );
		}
	}
}
add_action( 'wp_enqueue_scripts', 'tm_alt_styles' );

/**
 * Enqueue Fonts
 */
function tm_enqueue_fonts() {
    ...
    if ( ! empty( $theme_options['font'] ) || ! empty( $theme_options['font_alt'] ) ) {
        ...
        global $tm_google_fonts;
        $tm_google_fonts = $protocol . '://fonts.googleapis.com/css?family=' . $final_fonts;

        wp_enqueue_style( 'tm-custom-fonts', "$protocol://fonts.googleapis.com/css?family={$final_fonts}" );
    }
}

add_action( 'wp_enqueue_scripts', 'tm_enqueue_fonts' );